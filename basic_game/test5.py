            ...
            for point2 in self.points[i + 1:]:
                ...
                if distance < 10:  # Assume a radius of 10 for simplicity
                    ...
                    if velocity_along_normal < 0:  # Only bounce if moving towards each other
                        ...
                        point1.velocity -= impulse * point2.mass * normal
                        point2.velocity += impulse * point1.mass * normal
                ...
                for i, point in enumerate(engine.points):
                    if (point.position - mouse_pos).length() < 10:  # 10 is the click radius
                        if clicked_point is None:
                            # First click to drag and start a line
                            clicked_point = i
                        else:
                            # Second click to complete the line
                            engine.lines.append((clicked_point, i))
                            clicked_point = None  # Reset the first clicked point
                        break
